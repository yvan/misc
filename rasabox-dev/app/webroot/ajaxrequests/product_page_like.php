...
//because liketrack exists we know the tables exist and can just update them.
	...
	if ( strcmp($_POST['liked_or_disliked'] , "submitdislike") !== 0 ){
		...
		if ($_POST['user_liketable_query'] == 1 /*&& ($_POST['original_liketable_query'] != 2)*/){
		...
		else if ($_POST['user_liketable_query'] == 1 /*&& ($_POST['original_liketable_query'] == 2)*/){
			...
			"UPDATE `uploads` SET `likes` = '".($numberlikes)."', `dislikes` = '".($numberdislikes)."', `rank` = '".(round(log( ($numberlikes)-($numberdislikes), 3)*0.95))."', `true_rank` = '".(log( ($numberlikes)-($numberdislikes), 3)*0.95)."'
			WHERE `id` = '".$_POST['model_id']."';"
			...
			"UPDATE `mains` SET `likes` = '".($numberlikes)."', `dislikes` = '".($numberdislikes)."', `rank` = '".(round(log( ($numberlikes)-($numberdislikes), 3)*0.95))."', `true_rank` = '".(log( ($numberlikes)-($numberdislikes), 3)*0.95)."'
			WHERE `main_id` = '".$_POST['model_id']."';"
			...
			"UPDATE `products` SET `likes` = '".($numberlikes)."', `dislikes` = '".($numberdislikes)."', `rank` = '".(round(log( ($numberlikes)-($numberdislikes), 3)*0.95))."', `true_rank` = '".(log( ($numberlikes)-($numberdislikes), 3)*0.95)."'
			WHERE `product_id` = '".$_POST['model_id']."';"
	...
	if ( strcmp($_POST['liked_or_disliked'] , "submitlike") !== 0  ){
		...
		else if(($_POST['user_liketable_query'] == 2) /*&& ($_POST['original_liketable_query'] == 1)*/){
