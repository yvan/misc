...
class MainController extends AppController{
	...
	/*
	the index function that gets called when the page is loaded
	*/
	public function index(){
		...
		*/
		...
		foreach ($images as $image){
			...
			//instead of select * this could be select like_flag
			...
			//here we set it as 'user_liketable_query' in other files we set it as 'like_flag' a lot of places.
			if ($liketable_result){
		...
		//the reason this is else is ($search) and not jsut else is because
		//if we don't test for search to exist then what happens is
		//when we call a naked url (like just going to rasabox.com)
		//we get the problem that there is no search parameter passed
		//when this happens it goes to else and then enters the search (somehow)
		//probably a letter equal to null or 0, and then with searchflag set to one
		//for whatever reason the orange/blue colors don't dispaly properly later.
		//
		else if($search){
			...
			if (!empty($results)){
				...
				foreach ($results as $result) {
					...
					$searchnodes[$result['Upload']['id']]['title'] = str_replace("/", "&sect;", $result['Upload']['title']);
					$searchnodes[$result['Upload']['id']]['username'] = $result['Upload']['username'];
					$searchnodes[$result['Upload']['id']]['description'] = str_replace("/", "&sect;", $result['Upload']['description']);
					...
					if($search == null){
						...
						$searchnodes[$result['Upload']['id']]['search_value'] = $search; // from $this->params['named']['search'] is the search string the user has put in.
