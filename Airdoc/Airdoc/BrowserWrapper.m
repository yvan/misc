...
-(void) browser:(MCNearbyServiceBrowser *)browser foundPeer:(MCPeerID *)foreignPeerID withDiscoveryInfo:(NSDictionary *)info{
    ...
    NSLog(@"FOUND FOREIGN PEER: %@", [LocalStorageManager getPeerNameFromDisplayName: foreignPeerID.displayName]);
    
    //Automatically invite friend or stranger alike to session
...
-(void) browser:(MCNearbyServiceBrowser *)browser lostPeer:(MCPeerID *)foreignPeerID {
    ...
    NSLog(@"LOST FOREIGN PEER: %@", [LocalStorageManager getPeerNameFromDisplayName: foreignPeerID.displayName]);
