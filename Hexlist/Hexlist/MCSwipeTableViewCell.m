...
//    _tapGestureRecognizer.enabled = NO;
    ...
    CGPoint center = {_contentScreenshotView.center.x, _contentScreenshotView.center.y};
    _contentScreenshotView.center = center;
    
    UIView *view = [self viewWithPercentage:.25];
    [self setViewOfSlidingView:view];
    _slidingView.alpha = [self alphaWithPercentage:.25];
    ...
    NSString *animationUUID = [[NSUUID UUID] UUIDString];
    _animationUUID = animationUUID;
    
    [UIView animateWithDuration:_animationDuration delay:0.0 usingSpringWithDamping:_damping initialSpringVelocity:0.0 options:(UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionCurveEaseInOut) animations:^{
        
        CGRect frame = _contentScreenshotView.frame;
        frame.origin.x = [self offsetWithPercentage:.25 relativeToWidth:CGRectGetWidth(self.bounds)];
        _contentScreenshotView.frame = frame;
        
        // Clearing the indicator view
        _colorIndicatorView.backgroundColor = _color1;
        
    } completion:^(BOOL finished) {
        if (!_dragging && [_animationUUID isEqualToString:animationUUID]) {
...
- (void)swipeToOriginWithCompletion:(void(^)(void))completion {
    ...
    if ([UIView.class respondsToSelector:@selector(animateWithDuration:delay:usingSpringWithDamping:initialSpringVelocity:options:animations:completion:)]) {
        ...
        [UIView animateWithDuration:_animationDuration delay:0.0 usingSpringWithDamping:_damping initialSpringVelocity:_velocity options:(UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionCurveEaseInOut) animations:^{
